...
-- RLS Policies
CREATE POLICY "Users can view AI validation results for their applications"
ON ai_validation_results FOR SELECT
USING (
    EXISTS (
    ...
    OR EXISTS (
        ...
        AND profiles.role IN ('admin', 'officer', 'inspector', 'supervisor', 'revenue')
    )
);

CREATE POLICY "System can insert AI validation results"
ON ai_validation_results FOR INSERT
WITH CHECK (true);

CREATE POLICY "System can update AI validation results"
ON ai_validation_results FOR UPDATE
WITH CHECK (true);
...
USING (
    ...
    OR EXISTS (
        SELECT 1 FROM profiles
        WHERE profiles.id = auth.uid()
        AND profiles.role IN ('admin', 'officer', 'supervisor', 'revenue')
    )
);
...
-- RLS Policies
CREATE POLICY "Users can view generated documents for their applications"
...
-- RLS Policies
CREATE POLICY "Users can view workflow logs for their applications"
...
-- RLS Policies
CREATE POLICY "Users can view invoices for their applications"
...
CREATE POLICY "Revenue officers can insert invoices"
...
CREATE POLICY "Revenue officers can update invoices"
...
CREATE POLICY "Agents can insert payments"
...
CREATE POLICY "Revenue officers can update payments"

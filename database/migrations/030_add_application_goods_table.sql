...
CREATE POLICY "Allow read access to application_goods"
ON application_goods FOR SELECT
USING (
    EXISTS (
        ...
        AND (
            ...
            OR EXISTS (
                ...
                AND profiles.role IN ('admin', 'revenue_officer')
            )
        )
    )
);
...
CREATE POLICY "Allow agents to insert goods"
ON application_goods FOR INSERT
WITH CHECK (
    EXISTS (
        SELECT 1 FROM applications 
        WHERE applications.id = application_goods.application_id
...
CREATE POLICY "Allow agents to update goods"
ON application_goods FOR UPDATE
USING (
    EXISTS (
        SELECT 1 FROM applications 
        WHERE applications.id = application_goods.application_id
...
CREATE POLICY "Allow agents to delete goods"
ON application_goods FOR DELETE
USING (
    EXISTS (
        SELECT 1 FROM applications 
        WHERE applications.id = application_goods.application_id
